...
Choose two releases explicitly with a command such as `npm run benchmark:roswell-versions -- 100 1.3.68 1.3.67`. Roswell snapshots use the Dutch release where the strategy changed. A release without Roswell changes reuses the latest earlier snapshot, so Dutch 1.3.66 uses Roswell 1.3.65. Two releases that resolve to the same Roswell snapshot cannot be compared with each other.
