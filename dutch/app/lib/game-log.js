...
function finishedGameLogText(options = {}) {
  ...
  const output = [
    ...
    'Target: ' + (Number(options.roundLimit) === 5 ? 'Five rounds' : options.singleRound || Number(options.roundLimit) === 1 ? 'Single round' : options.gameTarget),
...
function saveFinishedGameLog(gameLogDir, gameState, winnerName, gameVersion = '', onError = console.error) {
  ...
  const content = finishedGameLogText({
    ...
    roundLimit: gameState.roundLimit || null,
