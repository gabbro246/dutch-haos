...
const { handForecast } = require('./bot-hand-forecast.js');
const { createTableForecast } = require('./bot-table-forecast.js');
...
const handKey = hand => hand.map(slot => slot.card ? key(slot.card) : `?:${slot.expected.toFixed(3)}:${slot.informed ? 1 : 0}`).sort().join(',');
...
const distributionIds = new WeakMap();
const compositionDistributions = new Map();
let nextDistributionId = 0;
function scoreDistribution(hand, limit = Infinity) {
  ...
  const composition = hand.map(slot => {
    if (slot.card) return 'K'+points(slot.card);
    if (!distributionIds.has(slot.distribution)) distributionIds.set(slot.distribution,++nextDistributionId);
    return 'U'+distributionIds.get(slot.distribution);
  }).sort().join(',')+'|'+limit;
  if (compositionDistributions.has(composition)) return compositionDistributions.get(composition);
  ...
  if (compositionDistributions.size >= 4096) compositionDistributions.clear();
  compositionDistributions.set(composition,result);
  return result;
}
...
function createSimpleTactics(deps) {
  ...
  const halvingDeferrals = new WeakMap();
  const table = createTableForecast(scoreDistribution);
  const ownAfter = (ctx, score) => scoreAfter(ctx.bot.total, score);
  ...
  function context(bot) {
    const current = state();
    const profile = deps.profileFor ? deps.profileFor(bot) : botProfile(bot);
    ...
    function slotsFor(player) {
      return deps.playerSlots(bot, player).map(slot => {
        const ownerKnows = slot.memory.ownerHasSeen === true ? 1 : slot.memory.ownerHasSeen === false ? 0 : slot.card ? 1 : .5;
        if (slot.card) return { ...slot, ownerKnows, expected: points(slot.card), distribution: [{ value: points(slot.card), probability: 1 }] };
        ...
        const tilt = selected ? (slot.memory.replacedKnown ? -0.20 : -0.12) : player.id === bot.id && profile.expectedUnknownValue !== 'counted' ? (ownMean - 6.5) * 0.10 : 0;
        const distribution = normalize(draw.map(item => ({ value: points(item.card), card: item.card, probability: item.probability * Math.exp(tilt * points(item.card)) * (selected && slot.memory.replacedKnown && Number.isFinite(slot.memory.replacedPoints) && points(item.card) > slot.memory.replacedPoints ? .35 : 1) })));
        return { ...slot, expected: player.id === bot.id ? ownMean : mean(distribution), distribution, ownerKnows: selected ? 1 : ownerKnows };
      });
    }
    ...
    const ctx = {
      bot, profile, players, own, others, hands, draw, roundNumber: current.roundNumber || 1, round: current.round || {},
      totalCards: draw.reduce((sum, item) => sum + item.count, 0),
      nodes: 0, budget: Math.max(100, profile.lookaheadNodeBudget || 1800),
      memo: new Map(), cycleMemo: new Map(), final: !!(current.round && current.round.dutchCallerId),
      target: current.gameTarget || 100, knowledge: profile.knowledgeInformationValue ?? profile.tacticalKnowledgeValue ?? 5,
      opponentWeight: profile.knowledgeOpponentWeight ?? profile.tacticalOpponentWeight ?? 0.55,
      threat: 0
    };
    const fingerprint = [ctx.final, ctx.round.dutchCallerId, draw.map(d => key(d.card)+':'+d.count).join(','),
      ...players.map(p => p.id+':'+p.total+':'+handKey(hands.get(p.id))+':'+hands.get(p.id).map(s => `${s.ownerKnows}:${!!s.memory?.publicSeen}`).join(','))].join('|');
    const previous = forecastCaches.get(bot);
    if (previous?.fingerprint === fingerprint) ctx.cycleMemo = previous.memo;
    else forecastCaches.set(bot, {fingerprint, memo:ctx.cycleMemo});
  ...
  function finishDiagnostics(ctx) {
    diagnostics.set(ctx.bot, { nodes: ctx.nodes, budget: ctx.budget, turns: ctx.profile.forecastHorizon === 'twoTurns' ? 2 : 3, plans: ctx.cycleMemo.size });
  }

  function withHands(ctx, hands) {
    const risks=ctx.others.map(p=>{
      const hand=hands.get(p.id);
      const low=scoreDistribution(hand,5).reduce((n,d)=>n+d.probability,0);
      return clamp(low*.8+(hand.length<=2?.2:0)+(hand.length<=1?.2:0));
    });
    return {...ctx,hands,memo:new Map(),threat:1-risks.reduce((p,r)=>p*(1-r),1)};
  }
  ...
  function throwRace(ctx, hand, discard, ownerId = ctx.bot.id) {
    ...
    for (const player of ctx.players) {
      ...
      for (const slot of slots) {
        const matching = !slot.card ? slot.distribution.filter(item => item.card?.rank === discard.rank && (!redKing(item.card) || slots.length === 1)) : [];
        const probability = slot.card ? (slot.card.rank === discard.rank && (!redKing(slot.card) || slots.length === 1) ? (slot.ownerKnows ?? .5) : 0)
          : matching.reduce((sum, item) => sum + item.probability, 0) * (slot.ownerKnows ?? 0.5);
        noMatch *= 1 - probability;
        if (probability > 0) benefit = Math.max(benefit, slot.card ? slot.expected : mean(normalize(matching)));
      }
      const match = 1 - noMatch;
      clearProbability *= 1 - match;
      const wins = player.isBot ? 0.5 : 1;
      none *= 1 - match * wins;
      const relevance = player.id === ctx.bot.id ? 1 : relevantWeight(ctx, player);
      rivalBenefit = Math.max(rivalBenefit, match * (benefit + (slots.length === 1 ? (ctx.profile.emptyHandGiftCost ?? 16) : 1)) * relevance);
    ...
    return { index: canOwn ? ownIndex : -1, probability: canOwn ? none : 0, clearProbability, opponentCost: rivalBenefit, rivalSpecial };
  }
  ...
  function callAssessment(ctx, hand = ctx.own, top = ctx.round.discard?.at(-1)) {
    ...
    if (score === 0) return { score, probability: 1, cost: ownAfter(ctx, 0), call: true };
    ...
    const cost = probability * ownAfter(ctx, 0) + (1 - probability) * ownAfter(ctx, 2 * score);
    const ordinary = ownAfter(ctx, score);
    ...
    const minimum = score <= 1 ? 0.55 : (ctx.profile.knowledgeCallMinimum ?? ctx.profile.tacticalCallMinimum ?? 0.80) - ctx.threat * 0.12;
  ...
  function halvingPlan(ctx, hand = ctx.own) {
    ...
    if ((!ctx.final || ctx.round.dutchCallerId === ctx.bot.id) && score > 5 && HALVING_TOTALS.includes(ctx.bot.total + score * 2)) {
      const result = ownAfter(ctx, score * 2);
  ...
  function ownTotalCost(ctx, hand, top) {
    ...
    let cost = distribution.reduce((sum, item) => sum + item.probability * ownAfter(ctx, item.value), 0);
    if (!ctx.final) {
      ...
      cost = distribution.reduce((sum, item) => {
        ...
        return sum + item.probability * (raw - (raw - ownAfter(ctx, item.value)) * finishChance);
      }, 0);
    ...
    if (ctx.round.dutchCallerId === ctx.bot.id) {
      if (score !== null && score > 5) cost = ownAfter(ctx, 2 * score);
      else if (score !== null) {
        let success = 1;
        ...
        cost = success * ownAfter(ctx, 0) + (1 - success) * ownAfter(ctx, 2 * score);
      }
    }
    return cost;
  }

  function completionWork(ctx, hand) {
    if (ctx.final) return 0;
    // Estimate useful actions still needed to know a hand of five or less.
    // Knowing a high card is progress: it can be thrown or deliberately replaced.
    const known = hand.filter(slot => slot.card).slice().sort((a, b) => a.expected - b.expected);
    let keptPoints = 0;
    const needsWork = [];
    for (const slot of known) {
      if (keptPoints + slot.expected <= 5) keptPoints += slot.expected;
      else needsWork.push(slot);
    }
    const ranks = new Map();
    let work = hand.filter(slot => !slot.card).length * (ctx.profile.knowledgeUnknownWork ?? 1.6);
    for (const slot of needsWork) {
      const rank = slot.card.rank;
      const opening = ctx.draw.filter(item => item.card.rank === rank).reduce((sum, item) => sum + item.probability, 0);
      work += 1 - Math.min(0.2, opening * ctx.others.length);
      ranks.set(rank, (ranks.get(rank) || 0) + 1);
    }
    // A known pair may be removed by one replacement and one matching throw.
    for (const count of ranks.values()) work -= Math.floor(count / 2) * 0.6;
    return work;
  }

  function projectedCost(ctx, hand, top) {
    const immediate = ownTotalCost(ctx, hand, top);
    if (ctx.final) return immediate;
    const model = handForecast(hand);
    if (!model) return immediate + (halvingPlan(ctx, hand) ? 0 : completionWork(ctx, hand) * 4);
    const call = callAssessment(ctx, hand, top);
    const plan = halvingPlan(ctx, hand);
    if (call.call && !plan) return call.cost;
    const cycleKey = `cycle:${top ? key(top) : '-'}:${ctx.others.map(p => handKey(ctx.hands.get(p.id))).join('/')}`;
    let cycle = ctx.memo.get(cycleKey);
    if (!cycle) { cycle = table.cycle(ctx, top); ctx.memo.set(cycleKey, cycle); }
    // A call still gives us a final turn. Development stops when the table is
    // likely to close; attacks are discounted for the repair on our next turn.
    let continuing = model[ctx.profile.forecastHorizon || 'threeTurns'];
    if (knownScore(hand) !== null && expected(hand) <= 5) continuing = model.finalTurn + .5;
    const evaluate = h => {
      const f=handForecast(h);if(!f)return expected(h);
      const ready=knownScore(h)!==null && expected(h)<=5;
      const future=ready ? f.finalTurn+.5 : f[ctx.profile.forecastHorizon || 'threeTurns'];
      return cycle.callChance*f.finalTurn+(1-cycle.callChance)*future;
    };
    let estimate = cycle.callChance * model.finalTurn + (1-cycle.callChance) * continuing;
    let throwGain=0;
    for(const opening of cycle.openings) {
      const at=matchingIndex(hand,opening.card.rank);
      if(at<0 || (redKing(hand[at].card) && hand.length>1))continue;
      const next=hand.filter((_,i)=>i!==at);
      let after=evaluate(next);
      if(hand[at].card.rank==='Q') {
        const unknown=next.find(s=>!s.card);
        if(unknown)after=unknown.distribution.reduce((v,d)=>v+d.probability*evaluate(next.map(s=>s===unknown?knownSlot(d.card,s):s)),0);
      }
      throwGain+=opening.probability * .7 * Math.max(0,estimate-after);
    }
    estimate = Math.max(0, estimate-throwGain) + cycle.attackCost * .5;
    const blend = ctx.profile.forecastImmediateWeight ?? 0;
    let cost = ctx.bot.total + blend * expected(hand) + (1-blend) * estimate;
    // Pursuing a halving and developing a different low hand are alternatives.
    if (plan?.type === 'wrong-dutch') cost = Math.min(cost, plan.result);
    if (plan?.type === 'ordinary') cost = Math.min(cost, plan.result + (1-cycle.callChance) * 3);
    return cost;
  }

  function futureJackValue(ctx, hand) {
    if (ctx.final || protectedTarget(ctx.bot.id)) return 0;
    const high = Math.max(0, ...hand.map(slot => slot.expected));
    let gain = 0;
    for (const p of ctx.others) if (!protectedTarget(p.id)) for (const slot of ctx.hands.get(p.id)) {
      if (slot.card) gain = Math.max(gain, high-slot.expected);
    }
    const chance = ctx.draw.filter(d => d.card.rank === 'J').reduce((p,d) => p+d.probability,0);
    return Math.max(0,gain) * (1-Math.pow(1-chance,2)) * (1-ctx.threat) * .8;
  }

  function ownPeekValue(ctx, hand, slot) {
    const model = handForecast(hand);
    let value = 0;
    if (!ctx.final && model) {
      const horizon = ctx.profile.forecastHorizon || 'threeTurns';
      value = model[horizon] - slot.distribution.reduce((v,d) => v+d.probability*handForecast(hand.map(s => s===slot ? knownSlot(d.card,s) : s))[horizon],0);
    } else if (!ctx.final) value = 3;
    if (ctx.round.throwIn?.open) value += slot.distribution.reduce((v,d) => v+(d.card.rank===ctx.round.throwIn.rank ? d.probability*(d.value+2) : 0),0);
    return Math.max(0,value);
  }

  function opponentPeekValue(ctx, hand, slot) {
    const high = Math.max(0, ...hand.map(s => s.expected));
    const knownGain = Math.max(0, ...ctx.others.filter(p => !protectedTarget(p.id)).flatMap(p => ctx.hands.get(p.id)).filter(s => s.card).map(s => high-s.expected));
    const optionGain = slot.distribution.reduce((v,d) => v + d.probability * Math.max(0, high-d.value-knownGain), 0);
    const jackChance = ctx.draw.filter(d => d.card.rank === 'J').reduce((v,d) => v+d.probability,0);
    const queuedJack = (ctx.round.specialQueue || []).some(s => s.actorId === ctx.bot.id && s.type === 'J');
    return optionGain * (queuedJack ? 1 : (1-Math.pow(1-jackChance,2)) * (1-ctx.threat));
  }

  function positionValue(ctx, hand, top, rivalDelta = 0) {
    const cacheKey = `value:${handKey(hand)}:${top ? key(top) : '-'}:${rivalDelta.toFixed(3)}`;
    if (ctx.memo.has(cacheKey)) return ctx.memo.get(cacheKey);
    const cost = projectedCost(ctx, hand, top);
    let value = -cost + rivalDelta * ctx.opponentWeight;
    if (!ctx.final) value += futureJackValue(ctx, hand);
    if (!ctx.final) {
      const terminal = terminalCallAssessment(ctx, hand, top);
      if (terminal?.call && terminal.score > 5) value = -terminal.cost + 150 * (2 * terminal.probability - 1);
    }
    ...
    if (ctx.final && ctx.hands.size && hand.every(slot => slot.card) && ctx.others.every(player => ctx.hands.get(player.id).every(slot => slot.card))) {
      ...
      const totals = ctx.players.map(player => {
        ...
        const roundPoints = player.id === ctx.round.dutchCallerId ? s <= 5 && s === min ? 0 : 2 * s : s;
        return { id: player.id, total: player.id === ctx.bot.id ? ownAfter(ctx, roundPoints) : scoreAfter(player.total, roundPoints) };
      });
    ...
    for (const opponent of ctx.others) {
      ...
      for (const b of ctx.hands.get(opponent.id)) {
        ...
        for (const a of hand) {
          ...
          const replaced = ctx.hands.get(opponent.id).map(slot => slot === b ? a : slot);
          const opponentGain = scoreDistribution(replaced).reduce((n,d) => n+d.probability*scoreAfter(opponent.total,d.value),0) - scoreDistribution(ctx.hands.get(opponent.id)).reduce((n,d) => n+d.probability*scoreAfter(opponent.total,d.value),0);
          const next = hand.map(slot => slot === a ? knownSlot(b.card, slot) : slot);
          const progress = projectedCost(ctx, hand, null) - projectedCost(ctx, next, null);
          const utility = progress + opponentGain * ctx.opponentWeight * relevantWeight(ctx, opponent) * repair;
          if (utility > 0) candidates.push({ a, b, type: 'self', utility, actionValue: utility, eligible: true });
        }
      }
    }
  ...
  function applyOwnSpecial(ctx, hand, card, rivalDelta) {
    ...
    if (card.rank === 'A') {
      ...
      return [{ ...branch, ctx: withHands(ctx, hands), rivalDelta: rivalDelta + attack.utility }];
    }
    if (card.rank === 'J') {
      ...
      if (swap.type !== 'self') {
        ...
        return [{ ...branch, ctx: withHands(ctx, hands), rivalDelta: rivalDelta + swap.utility / ctx.opponentWeight }];
      }
      const next = hand.map(slot => slot === swap.a ? knownSlot(swap.b.card, slot) : slot);
      hands.set(swap.b.player.id, hands.get(swap.b.player.id).map(slot => slot.index === swap.b.index ? { ...swap.a, player: swap.b.player, index: slot.index } : slot));
      return [{ ...branch, hand: next, ctx: withHands(ctx, hands), rivalDelta: rivalDelta + (swap.a.expected - swap.b.expected) * relevantWeight(ctx, swap.b.player) * 0.65 }];
    }
    const unknowns = hand.filter(slot => !slot.card && !slot.informed);
    if (!unknowns.length) {
      const targets = ctx.others.filter(p => !protectedTarget(p.id)).flatMap(p => ctx.hands.get(p.id).filter(s => !s.card));
      const selected = targets.sort((a,b) => opponentPeekValue(ctx,hand,b)-opponentPeekValue(ctx,hand,a))[0];
      if (!selected) return [branch];
      return [{ ...branch, rivalDelta: rivalDelta + opponentPeekValue(ctx, hand, selected) / ctx.opponentWeight }];
    }
    const selected = unknowns.reduce((a, b) => ownPeekValue(ctx,hand,a) >= ownPeekValue(ctx,hand,b) ? a : b);
    // Evaluate what every possible reveal enables, including newly known pairs
    // and call-ready hands. A peek never leaves an artificial 'known average'.
  ...
  function specialOutcome(ctx, hand, discard, rivalDelta, thrown = null, openProbability = 0) {
    ...
    return branches.reduce((sum, branch) => {
      const stay = positionValue(branch.ctx, branch.hand, thrown || discard, branch.rivalDelta);
      if (thrown || !openProbability || !special(discard)) return sum + branch.probability * stay;
      const index = matchingIndex(branch.hand, discard.rank);
      const plan = halvingPlan(branch.ctx, branch.hand);
      if (index < 0 || (plan && knownScore(branch.hand) - branch.hand[index].expected !== plan.desiredHandScore)) return sum + branch.probability * stay;
      // A Queen reveal or Jack swap can discover a match while the original
      // opening is still live. Remove it once, then resolve its own special.
      const card = branch.hand[index].card;
      const next = branch.hand.filter((_, i) => i !== index);
      const followups = applyOwnSpecial(branch.ctx, next, card, branch.rivalDelta);
      const throwValue = followups.reduce((value, follow) => value + follow.probability * positionValue(follow.ctx, follow.hand, card, follow.rivalDelta), 0);
      return sum + branch.probability * (stay + openProbability * Math.max(0, throwValue - stay));
    }, 0);
  }
  ...
  function actionValue(ctx, hand, incoming, index, options = {}) {
    ...
    for (const outcome of outgoing) {
      ...
      const noOwn = specialOutcome(ctx, next, discard, -race.opponentCost, null, probability ? 0 : race.clearProbability);
      let own = noOwn;
      if (probability) {
      ...
      if (!ctx.final && !options.future && ctx.nodes < ctx.budget) {
        ...
        if (nextPlayer && points(discard) <= 5) {
          ...
          utility -= outcome.probability * gift * relevantWeight(ctx, nextPlayer) * 0.12 * (ctx.profile.opponentPilePickupWeight ?? 1);
        }
      }
    }
    return { index, target: index < 0 ? null : hand[index], utility, actionValue: utility, improvement: index < 0 ? 0 : hand[index].expected - points(incoming), incoming, next };
  ...
  function planActions(ctx, incoming, required = false) {
    ...
    if (ctx.final || (ctx.profile.knowledgeFutureTurns ?? 1) < 2 || ctx.threat > 0.75) return actions;
  ...
  function evaluateDrawSources(bot) {
    ...
    // The forecast already prices future information. Do not add the old
    // four-point pile surcharge and count the same learning opportunity twice.
    const margin = ctx.final ? .1 : (ctx.profile.forecastPileMargin ?? .25);
  ...
  function botQueenTargets(bot) {
    ...
    const ownUnknown = canUseKnowledge ? ctx.own.filter(slot => !slot.card).map(slot => ({ ...slot, utility: ownPeekValue(ctx,ctx.own,slot) })) : [];
    const opponentUnknown = canUseKnowledge && (!ctx.final || (ctx.round.specialQueue || []).some(s => s.actorId===bot.id && s.type==='J')) ? ctx.others.flatMap(player => ctx.hands.get(player.id).filter(slot => !slot.card).map(slot => ({ ...slot, danger: relevantWeight(ctx, player), utility: opponentPeekValue(ctx, ctx.own, slot) + .1 * relevantWeight(ctx, player) }))) : [];
    return { ownUnknown, opponentUnknown };
  }

  function canDelayHalvingGift(ctx) {
    const score = knownScore(ctx.own);
    if (score === null || score > 2 || !ctx.own.length) return false;
    const threatened = ctx.others.some(p => {
      const handScore = knownScore(ctx.hands.get(p.id));
      if (handScore === null || handScore <= score || !HALVING_TOTALS.includes(p.total+handScore)) return false;
      return (p.total+handScore)/2 <= ctx.bot.total+15;
    });
    if (!threatened) return false;
    let deferral = halvingDeferrals.get(ctx.bot);
    if (!deferral || deferral.round !== ctx.roundNumber) {
      deferral={round:ctx.roundNumber,startTick:ctx.round.strategyTick || 0};
      halvingDeferrals.set(ctx.bot,deferral);
    }
    // A short window to draw an Ace or useful Jack. Never wait indefinitely
    // for a counter-card, and never sacrifice a certain whole-game win.
    return (ctx.round.strategyTick || 0)-deferral.startTick < ctx.players.length*(ctx.profile.halvingDenialTurns ?? 2);
  }

  function botShouldCallDutch(bot) {
    const ctx = context(bot);
    if (ctx.final) return false;
    const terminal = terminalCallAssessment(ctx);
    const plan = halvingPlan(ctx);
    const result = terminal?.call ? terminal : plan?.type === 'wrong-dutch'
      ? { call: true, score: knownScore(ctx.own), cost: plan.result } : callAssessment(ctx);
    if (plan?.type === 'ordinary' && !terminal?.call) return false;
    finishDiagnostics(ctx);
    const pending = ctx.round.stage === 'special' && ctx.round.specialQueue?.[0];
    if (result.call && pending?.actorId === bot.id) {
      // Resolving an already-owned special adds no normal turn. Compare its
      // value even at zero points or before a deliberate halving call.
      const card = { rank: pending.type, suit: 'clubs' };
      if (specialOutcome(ctx, ctx.own, card, 0) > -result.cost + .25) return false;
    }
    if (result.call && !terminal?.call && !plan && canDelayHalvingGift(ctx)) return false;
    return result.call;
  }
  ...
  function botThrowInCandidate(bot) {
    ...
    const candidates = ctx.own.filter(slot => slot.card?.rank === ctx.round.throwIn.rank).filter(slot => !redKing(slot.card) || ctx.own.length === 1).filter(slot => {
      ...
      return ownAfter(ctx, 2 * score) <= ctx.target || ownAfter(ctx, 2 * (score - slot.expected)) > ctx.target;
    });
