...
function createSimpleTactics(deps) {
  ...
  function finishDiagnostics(ctx) {
    diagnostics.set(ctx.bot, { nodes: ctx.nodes, budget: ctx.budget, turns: ctx.profile.knowledgeFutureTurns ?? 1, plans: 0 });
  }
  ...
  function throwRace(ctx, hand, discard, ownerId = ctx.bot.id) {
    ...
    for (const player of ctx.players) {
      ...
      for (const slot of slots) {
        const matching = !slot.card ? slot.distribution.filter(item => item.card?.rank === discard.rank && !redKing(item.card)) : [];
  ...
  function positionValue(ctx, hand, top, rivalDelta = 0) {
    ...
    let value = -cost - unknowns * knowledge + rivalDelta * ctx.opponentWeight
      - completionWork(ctx, hand) * (ctx.profile.knowledgeWorkWeight ?? 8);
    if (!ctx.final) {
    ...
    for (const opponent of ctx.others) {
      ...
      for (const b of ctx.hands.get(opponent.id)) {
        ...
        for (const a of hand) {
          ...
          const progress = (completionWork(ctx, hand) - completionWork(ctx, next)) * (ctx.profile.knowledgeWorkWeight ?? 8);
          const utility = gain + knowledge + progress + opponentGain * ctx.opponentWeight * relevantWeight(ctx, opponent) * repair;
  ...
  function applyOwnSpecial(ctx, hand, card, rivalDelta) {
    ...
    if (!unknowns.length) return [branch];
  ...
  function evaluateDrawSources(bot) {
    ...
    const margin = !ctx.final && ctx.own.some(slot => !slot.card) ? (ctx.profile.knowledgePileMargin ?? ctx.profile.tacticalPileMargin ?? 0.35) : 0.1;
